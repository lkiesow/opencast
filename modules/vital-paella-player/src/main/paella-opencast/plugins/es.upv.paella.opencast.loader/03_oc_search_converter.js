...
class OpencastToPaellaConverter {
  ...
  getCaptions(episode) {
    ...
    var catalogs = [];
    if (episode.mediapackage)  {
      catalogs = episode.mediapackage.metadata.catalog;
    }
    if (!(attachments instanceof Array)) { attachments = attachments ? [attachments] : []; }
  ...
  convertToDataJson(episode) {
    ...
    var data =  {
      metadata: {
        title: episode.mediapackage ? episode.mediapackage.title : 'title',
        duration: episode.mediapackage ? episode.mediapackage.duration / 1000 : 0,
