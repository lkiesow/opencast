...
public class OaiPmhPersistenceTest {
  ...
  public void testDeleting() throws Exception {
    ...
    SearchResult search = oaiPmhDatabase.search(query().mediaPackageId(mp1));
  ...
  public void testRetrieving() throws Exception {
    ...
    search = oaiPmhDatabase.search(query().mediaPackageId(mp1));
    Assert.assertEquals(2, search.size());

    search = oaiPmhDatabase.search(query().mediaPackageId(mp1).repositoryId(REPOSITORY_ID_2));
    Assert.assertEquals(1, search.size());
    ...
    search = oaiPmhDatabase.search(query().mediaPackageId(mp1).repositoryId(REPOSITORY_ID_1));
    Assert.assertEquals(1, search.size());
    ...
    search = oaiPmhDatabase
            .search(query().mediaPackageId(mp1).repositoryId(REPOSITORY_ID_2).modifiedAfter(new Date()));
    ...
    search = oaiPmhDatabase.search(query().modifiedAfter(dateBeforeStoring));
    Assert.assertEquals(1, search.size());
    ...
    search = oaiPmhDatabase.search(query().modifiedAfter(new Date()));
    Assert.assertEquals(0, search.size());

    search = oaiPmhDatabase.search(query().modifiedAfter(dateBeforeDeletion));
    Assert.assertEquals(1, search.size());
  }
  ...
  public void testLimitOffset() throws Exception {
    ...
    SearchResult search = oaiPmhDatabase.search(query().limit(2));
    Assert.assertEquals(2, search.size());

    search = oaiPmhDatabase.search(query().limit(1));
    Assert.assertEquals(1, search.size());
    Assert.assertEquals(mp1.getIdentifier().toString(), search.getItems().get(0).getId());

    search = oaiPmhDatabase.search(query().offset(1));
    Assert.assertEquals(1, search.size());
  ...
  public void testRemovalOfOrphanedElements() throws Exception {
    ...
    for (SearchResultItem searchResultItem : oaiPmhDatabase
            .search(queryRepo(REPOSITORY_ID_1).mediaPackageId(mp1)).getItems()) {
      count++;
