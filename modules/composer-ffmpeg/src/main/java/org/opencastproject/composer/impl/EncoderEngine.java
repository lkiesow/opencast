...
public class EncoderEngine implements AutoCloseable {
  ...
  /**
   * Encode the videoSource with an encoding profile that produces multiple outputs Care is taken that the files are
   * returned in the order listed in the profile
   *
   * @param videoSource
  ...
  private List<File> process(List<String> command, List<File> inputs, List<String> outputs,
    ...
    try {
      ...
      String line;
      while ((line = in.readLine()) != null) {
        handleEncoderOutput(new ArrayList<>(), line);
      }
      // wait until the task is finished
